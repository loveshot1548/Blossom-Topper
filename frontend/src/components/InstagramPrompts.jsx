...
export default function InstagramPrompts() {
  ...
  return (
    <div className="animate-fade-in space-y-6 max-w-7xl mx-auto px-2 sm:px-0">
      <div className="bg-white/85 backdrop-blur-xl p-5 sm:p-6 md:p-8 rounded-3xl border border-green-800/20 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="bg-green-100 text-green-800 text-xs font-bold px-3 py-1 rounded-full">INSTAGRAM PROMPTS 100</span>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight mt-2">인스타그램 프롬프트 모음 아카이브</h1>
          <p className="text-sm sm:text-base text-gray-600 mt-1">원하는 프롬프트를 검색하거나 카테고리별로 골라 클릭 한 번에 복사하세요.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredPrompts.length > 0 ? (
          filteredPrompts.map((item) => (
            <div key={item.id} className="bg-white/85 backdrop-blur-xl rounded-3xl p-5 sm:p-6 border border-green-800/20 shadow-sm flex flex-col justify-between transition-all hover:shadow-md">
              <div>
                <div className="flex justify-between items-start mb-3 gap-2">
                  <h2 className="text-base sm:text-lg font-bold text-gray-900 break-words">{item.title}</h2>
                  <span className="text-xs bg-gray-100 text-gray-600 font-semibold px-2.5 py-1 rounded-lg shrink-0">ID: #{item.id}</span>
                </div>
                <div className="bg-gray-50/80 border border-gray-200/80 rounded-2xl p-4 text-xs sm:text-sm text-gray-700 whitespace-pre-line font-mono leading-relaxed mb-4 max-h-48 overflow-y-auto custom-scrollbar break-words">
                  {item.text}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pt-3 border-t border-gray-100">
                ...
                <button
                  ...
                  className={`w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm ${
